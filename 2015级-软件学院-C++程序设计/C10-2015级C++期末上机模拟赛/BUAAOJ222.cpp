...
string bigIntegerAdd(string s1, string s2) {
    ...
    for (int i = 0; i < maxLen; ++i) {
        if (debug)cout << i << " : " << a[i] << " " << b[i] << " " << a[i] + b[i] << endl;
        if (a[i] + b[i] >= 10) {
            int tmp = a[i] + b[i];
            a[i] = tmp % 10;
            a[i + 1] += tmp / 10;
        ...
        if (debug)cout << i << " : " << a[i] << endl;
    }
    string ret = "";
    if (a[maxLen] != 0)ret += a[maxLen] + '0';
    for (int i = maxLen - 1; i >= 0; --i)ret += a[i] + '0';
    return ret;
}
